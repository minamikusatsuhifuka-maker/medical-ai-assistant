...
const preprocessText=(text)=>{return text.replace(/\n{3,}/g,"\n\n").replace(/[ \t]+/g," ").replace(/(.{2,}?)\1{2,}/g,"$1").replace(/^(えー|あー|うーん|えっと|まあ|あの|その|ええ)[、。\s]*/gm,"").trim()};
...
const cleanedInput=preprocessText(t);
...
setProg(40);
const response=await fetch("/api/summarize?stream=1",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:cleanedInput,mode:"gemini",prompt:enhancedPrompt})});
if(response.headers.get("content-type")?.includes("text/event-stream")){const reader=response.body.getReader();const decoder=new TextDecoder();let result="";let currentModel="";while(true){const{done,value}=await reader.read();if(done)break;const chunk=decoder.decode(value,{stream:true});const lines=chunk.split("\n");for(const line of lines){if(line.startsWith("data: ")){try{const data=JSON.parse(line.slice(6));if(data.done){currentModel=data.model||""}else if(data.text){result+=data.text;sOut(result);if(data.model)currentModel=data.model}}catch{}}}}if(currentModel)setGeminiModel(currentModel);setProg(90);sumDoneRef.current=true;await saveRecord(t,result);try{await navigator.clipboard.writeText(result);sSt(`要約完了 ✓ [${currentModel||"gemini"}]`)}catch{sSt(`要約完了 [${currentModel||"gemini"}]`)}}else{const d=await response.json();if(d.error){sSt("エラー: "+d.error);return}if(d.model)setGeminiModel(d.model);setProg(90);sOut(d.summary);sumDoneRef.current=true;await saveRecord(t,d.summary);try{await navigator.clipboard.writeText(d.summary);sSt(`要約完了 ✓ [${d.model||"gemini"}]`)}catch{sSt(`要約完了 [${d.model||"gemini"}]`)}}}catch{sSt("エラーが発生しました")}finally{sLd(false);setProg(0)}};
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h2 style={{fontSize:18,fontWeight:700,color:"#3f6212",margin:0}}>📖 使い方ガイド</h2><button onClick={()=>{setPage("main")}} style={btn(C.p,C.pDD)}>✕ 閉じる</button></div>
...
{rs==="inactive"?(<button onClick={go} style={{...rb,width:mob?76:90,height:mob?76:90,background:"linear-gradient(135deg,#3f6212,#65a30d)",color:C.w,boxShadow:"0 4px 14px rgba(101,163,13,.25)"}}><span style={{fontSize:30}}>🎙</span><span style={{fontSize:12}}>録音開始</span></button>):(<>
