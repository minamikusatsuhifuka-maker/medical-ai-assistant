...
{isTrans&&<span style={{fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:5,background:"#ede9fe",color:"#7c3aed"}}>📝 書き起こし</span>}
{isSum&&<span style={{fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:5,background:"#dcfce7",color:"#166534"}}>📋 要約</span>}
</div>
...
{isTrans?<button onClick={()=>setFavDetailModal({...f,title:"📝 書き起こし"})} title="書き起こし内容を表示" onMouseEnter={e=>showTip(e,"書き起こし内容を表示")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:"1px solid #c4b5fd",background:"#ede9fe",fontSize:11,fontWeight:600,color:"#7c3aed",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📝書起</button>:isSum?<button onClick={()=>setFavDetailModal({...f,title:"📋 要約"})} title="要約内容を表示" onMouseEnter={e=>showTip(e,"要約内容を表示")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:"1px solid rgba(160,220,100,0.4)",background:"rgba(220,252,231,0.6)",fontSize:11,fontWeight:600,color:"#166534",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📋要約</button>:<button onClick={()=>setFavDetailModal(f)} title="内容を表示" onMouseEnter={e=>showTip(e,"内容を表示")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:`1px solid ${C.g200}`,background:C.g50,fontSize:11,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📋内容</button>}
...
<button onClick={()=>{setQcModal({input_text:f.content||""});setQcResult("");setQcLoading(true);fetch("/api/quality-check",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:f.content||""})}).then(r=>r.json()).then(d=>{if(d.error)throw new Error(d.error);setQcResult(d.result||"")}).catch(e=>setQcResult("エラー: "+e.message)).finally(()=>setQcLoading(false))}} title="AIで内容を分析" onMouseEnter={e=>showTip(e,"AIで内容を分析")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:"1px solid #93c5fd",background:"#eff6ff",fontSize:11,fontWeight:600,color:"#2563eb",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>🔍分析</button>
