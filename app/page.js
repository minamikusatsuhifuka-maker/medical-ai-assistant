...
{hasSum&&<button onClick={()=>setFavDetailModal({...f,title:"📋 要約",content:extractSum()})} title="要約内容を表示" onMouseEnter={e=>showTip(e,"要約内容を表示")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:`1px solid ${C.p}`,background:C.pLL,fontSize:11,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📋要約</button>}
{!hasTrans&&!hasSum&&<button onClick={()=>setFavDetailModal(f)} title="内容を表示" onMouseEnter={e=>showTip(e,"内容を表示")} onMouseLeave={hideTip} style={{padding:"4px 12px",borderRadius:6,border:`1px solid ${C.g200}`,background:C.g50,fontSize:11,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📋内容</button>}
