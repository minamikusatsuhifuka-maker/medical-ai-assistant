...
<button onClick={async(e)=>{e.stopPropagation();setDailyTypoLd(dk);setTypoTarget("hist");const CHUNK_SIZE=2000;const fullText=recs.map(r=>[r.input_text||"",r.output_text||""].filter(Boolean).join("\n")).join("\n---\n");const chunks=[];for(let i=0;i<fullText.length;i+=CHUNK_SIZE)chunks.push(fullText.slice(i,i+CHUNK_SIZE));const allCorrections=[];const seenFroms=new Set();let errCount=0;try{for(let ci=0;ci<chunks.length;ci++){sSt(chunks.length>1?`🔬 スキャン中... (${ci+1}/${chunks.length})`:`🔬 ${dk} スキャン中...`);try{const res=await fetch("/api/fix-typos",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:chunks[ci],dictionary:dict.map(([from,to])=>({from,to}))})});if(!res.ok){const errText=await res.text();console.error("API error response chunk "+ci+":",errText);errCount++;continue}const d=await res.json();if(d.error){console.error("Chunk "+ci+" error:",d.error);errCount++;continue}if(d.corrections&&d.corrections.length>0){d.corrections.forEach(c=>{if(seenFroms.has(c.from)){const idx=allCorrections.findIndex(x=>x.from===c.from);if(idx!==-1)allCorrections[idx]=c}else{seenFroms.add(c.from);allCorrections.push(c)}})}}catch(chunkErr){console.error("Chunk "+ci+" fetch error:",chunkErr);errCount++}}if(allCorrections.length===0){if(errCount===chunks.length){sSt("校正エラー: 全チャンクでエラーが発生しました")}else{sSt("✓ 医療用語の誤りは見つかりませんでした")}return}const sel={};allCorrections.forEach((c,i)=>{if(c.candidates&&c.candidates.length===1)sel[i]=0});setTypoSelections(sel);setTypoCustomInputs({});setTypoModal(allCorrections);sSt(chunks.length>1?`✓ ${allCorrections.length}件の校正候補が見つかりました（${chunks.length}回に分けてスキャン）`:"校正候補が見つかりました")}catch(err){console.error("Daily typo scan error:",err);sSt("校正エラー: "+err.message)}finally{setDailyTypoLd(null)}}} title="この日の診療記録をAI誤字スキャン" onMouseEnter={e=>showTip(e,"この日の診療記録をAI誤字スキャン")} onMouseLeave={hideTip} disabled={dailyTypoLd===dk} style={{padding:"4px 10px",borderRadius:8,border:"1px solid rgba(160,220,100,0.3)",background:dailyTypoLd===dk?"#e5e7eb":"rgba(255,255,255,0.5)",fontSize:11,fontWeight:600,color:dailyTypoLd===dk?C.g400:"#2a5018",fontFamily:"inherit",cursor:dailyTypoLd===dk?"wait":"pointer",whiteSpace:"nowrap"}}>{dailyTypoLd===dk?"🔬 スキャン中...":"🔬 誤字スキャン"}</button>
