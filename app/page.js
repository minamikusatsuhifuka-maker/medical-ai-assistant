...
{typoSelections[i]!==undefined&&<button onClick={()=>{applyTypoCorrection(i,typoSelections[i]);setTypoModal(prev=>{if(!prev)return null;const n=[...prev];n.splice(i,1);if(!n.length)return null;return n});setTypoSelections(prev=>{const ns={};Object.keys(prev).forEach(k=>{const ki=Number(k);if(ki<i)ns[ki]=prev[ki];else if(ki>i)ns[ki-1]=prev[ki]});return ns});sSt("✓ 修正+辞書登録しました")}} style={{padding:"4px 14px",borderRadius:8,border:"none",background:C.rG,color:C.w,fontSize:12,fontWeight:700,fontFamily:"inherit",cursor:"pointer"}}>✓ これで登録</button>}
</div>))}
