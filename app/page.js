...
const sum=async(tx)=>{const t=tx||iR.current;if(!t.trim()){sSt("テキストを入力してください");return}sumDoneRef.current=false;sLd(true);setProg(10);sSt("Gemini で要約中...");try{
...
pw.document.getElementById("pip-sum").onclick=()=>{sumDoneRef.current=false;stopSum();setTimeout(pipBtnUpdate,500);const d=pipRef.current;if(d){try{const old=d.getElementById("pip-alert");if(old)old.remove()}catch{};const bar=d.createElement("div");bar.id="pip-progress";bar.style.cssText="position:fixed;top:0;left:0;width:100%;height:4px;z-index:9999;background:#e7e5e4";const inner=d.createElement("div");inner.style.cssText="height:100%;width:10%;background:linear-gradient(90deg,#84cc16,#22c55e);border-radius:2px;transition:width 0.5s ease";bar.appendChild(inner);d.body.appendChild(bar);let pct=10;const progI=setInterval(()=>{if(sumDoneRef.current){pct=100}else if(pct<85){pct+=Math.random()*8+2}inner.style.width=pct+"%"},500);const checkDone=setInterval(()=>{if(sumDoneRef.current){clearInterval(checkDone);clearInterval(progI);inner.style.width="100%";setTimeout(()=>{try{bar.remove()}catch{}const outputText=oR.current||"";if(outputText.trim()){navigator.clipboard.writeText(outputText).then(()=>{try{const old2=d.getElementById("pip-alert");if(old2)old2.remove()}catch{};const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了・📋コピー済み";d.body.appendChild(div)}).catch(()=>{const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了";d.body.appendChild(div)})}else{const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了";d.body.appendChild(div)}},600)}},500)}};
