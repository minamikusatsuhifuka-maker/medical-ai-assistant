...
pw.document.getElementById("pip-sum").onclick=()=>{const d=pipRef.current;if(!d)return;const copyAndAlert=(text,doc)=>{try{const ta=document.createElement("textarea");ta.value=text;ta.style.cssText="position:fixed;left:-9999px;top:-9999px";document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);const div=doc.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了・📋コピー済み";doc.body.appendChild(div)}catch(e){try{navigator.clipboard.writeText(text)}catch{};const div=doc.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了・📋コピー済み";doc.body.appendChild(div)}};sumDoneRef.current=false;try{const old=d.getElementById("pip-alert");if(old)old.remove();const oldBar=d.getElementById("pip-progress");if(oldBar)oldBar.remove()}catch{};const bar=d.createElement("div");bar.id="pip-progress";bar.style.cssText="position:fixed;top:0;left:0;width:100%;height:5px;z-index:9999;background:#e7e5e4";const inner=d.createElement("div");inner.style.cssText="height:100%;width:5%;background:linear-gradient(90deg,#84cc16,#22c55e);border-radius:2px;transition:width 0.3s ease";bar.appendChild(inner);d.body.appendChild(bar);const loading=d.createElement("div");loading.id="pip-loading";loading.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#f59e0b;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";loading.textContent="⏳ 要約中...";d.body.appendChild(loading);stopSum();setTimeout(pipBtnUpdate,500);let pct=5;const progI=setInterval(()=>{if(sumDoneRef.current){pct=100;inner.style.width="100%";clearInterval(progI)}else if(pct<90){pct+=2;inner.style.width=pct+"%"}},300);const checkDone=setInterval(()=>{if(sumDoneRef.current){clearInterval(checkDone);clearInterval(progI);inner.style.width="100%";try{const ld=d.getElementById("pip-loading");if(ld)ld.remove()}catch{};setTimeout(()=>{try{bar.remove()}catch{};const outputText=oR.current||"";if(outputText.trim()){try{const old2=d.getElementById("pip-alert");if(old2)old2.remove()}catch{};copyAndAlert(outputText,d)}else{const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了";d.body.appendChild(div)}},1000)}},500)};
