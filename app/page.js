...
export default function Home() {
  ...
  const startRecording = async () => {
    ...
    const startNewChunk = () => {
      if (mediaRecRef.current && mediaRecRef.current.state === "recording") {
        mediaRecRef.current.stop(); // This triggers ondataavailable → transcribe
      }
    ...
    startNewChunk();
    // Auto-chunk every 30 seconds to stay under 25MB and get frequent results
  ...
  const stopRecording = () => {
    ...
    if (mediaRecRef.current && mediaRecRef.current.state === "recording") {
      mediaRecRef.current.stop(); // Final chunk → transcribe
    }
