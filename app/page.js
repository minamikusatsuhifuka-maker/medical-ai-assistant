...
setProg(50);
const r=await fetch("/api/summarize",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:sysPrompt,mode:"gemini",prompt:"以下の指示に従って患者向け説明資料を作成してください。"})});const d=await r.json();if(d.error){setDocOut("エラー: "+d.error)}else{setDocOut(`[使用モデル: ${d.model||"gemini"}]\n\n${d.summary}`)}}catch(e){setDocOut("エラー: "+e.message)}finally{setDocLd(false);setProg(0)}};
...
const prompt=`${p}\n\n【書き起こし内容】\n${minIR.current}\n\n以下の構成で簡潔にまとめてください：\n1. 日時・参加者（わかる場合）\n2. 議題・アジェンダ\n3. 決定事項\n4. 各議題の要点\n5. アクションアイテム（担当者・期限）\n6. 次回予定`;
setProg(50);
try{const r=await fetch("/api/summarize",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:prompt,mode:"gemini",prompt:"議事録を作成してください。"})});const d=await r.json();if(d.error){setMinOut("エラー: "+d.error)}else{setMinOut(`[使用モデル: ${d.model||"gemini"}]\n\n${d.summary}`)}}catch(e){setMinOut("エラー: "+e.message)}finally{setMinLd(false);setProg(0)}};
...
setProg(50);
try{const r=await fetch("/api/summarize",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:modes[csMode],mode:"gemini",prompt:"詳細に分析してください。"})});const d=await r.json();setCsOut(d.error?"エラー: "+d.error:`[使用モデル: ${d.model||"gemini"}]\n\n${d.summary}`)}catch(e){setCsOut("エラー: "+e.message)}finally{setCsLd(false);setProg(0)}};
...
<div style={{display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:9,color:"rgba(255,255,255,.45)",fontWeight:500}}>Gemini 3 Flash</span>{pc>0&&<span style={{fontSize:12,color:C.warn,fontWeight:600}}>⏳</span>}<span style={{fontSize:11,color:st.includes("✓")?"#86efac":"rgba(255,255,255,.7)",fontWeight:st.includes("✓")?600:400}}>{st}</span></div></header>
{prog>0&&<div style={{width:"100%",height:4,background:C.g200,borderRadius:2,marginBottom:8,overflow:"hidden"}}><div style={{width:`${prog}%`,height:"100%",background:`linear-gradient(90deg,${C.p},${C.rG})`,borderRadius:2,transition:"width 0.5s ease"}}/></div>}
