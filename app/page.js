...
pw.document.getElementById("pip-rec").onclick=()=>{if(iR.current&&iR.current.trim()){const d=pipRef.current;if(d){try{const al=d.getElementById("pip-alert");if(al)al.remove()}catch{}const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;top:10px;left:50%;transform:translateX(-50%);background:#f59e0b;color:#fff;padding:10px 20px;border-radius:12px;font-size:13px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.2)";div.innerHTML="⚠️ 前のカルテが残っています<br>「次へ」で消してから録音開始してください";d.body.appendChild(div);setTimeout(()=>{try{div.remove()}catch{}},4000)}return}go();setTimeout(pipBtnUpdate,500)};
...
pw.document.getElementById("pip-sum").onclick=()=>{stopSum();setTimeout(pipBtnUpdate,500);const waitDone=setInterval(()=>{const d=pipRef.current;if(d&&!document.querySelector('[style*="spin"]')){clearInterval(waitDone);try{const al=d.getElementById("pip-alert");if(al)al.remove();const div=d.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;top:10px;left:50%;transform:translateX(-50%);background:#22c55e;color:#fff;padding:10px 20px;border-radius:12px;font-size:13px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.2)";div.innerHTML="✅ 要約完了！<br>📋 コピーしました";d.body.appendChild(div);setTimeout(()=>{try{div.remove()}catch{}},3000)}catch{}}},1500)};
