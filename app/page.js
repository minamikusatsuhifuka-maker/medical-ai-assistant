...
const bulkSaveFavorites=async(group)=>{if(!supabase)return;const selected=filteredHist.filter(r=>selectedHistIds.has(r.id));if(!selected.length)return;try{const rows=selected.map(r=>{const date=r.created_at?new Date(r.created_at).toLocaleDateString("ja-JP"):"";const pid=r.patient_id||"";const content=r.input_text&&r.output_text?`【書き起こし】\n${r.input_text}\n\n【要約】\n${r.output_text}`:r.output_text?`【要約】\n${r.output_text}`:r.input_text?`【書き起こし】\n${r.input_text}`:"";return{record_id:r.id||"",group_name:group,title:date+(pid?" | "+pid:""),content}});await supabase.from("favorites").insert(rows);setFavToast(`⭐ ${selected.length}件を${group}グループに保存しました`);setTimeout(()=>setFavToast(""),3000);loadFavorites()}catch(e){console.error("Bulk fav save error:",e)}};
...
<button onClick={()=>setHistPopup({title:"📝 書き起こし",content:r.input_text||"（書き起こしなし）",date,pid})} title="書き起こし内容を表示" style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📝書起</button>
<button onClick={()=>setHistPopup({title:"📋 要約",content:r.output_text||"（要約なし）",date,pid})} title="要約内容を表示" style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.p}`,background:C.pLL,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📋要約</button>
<button onClick={()=>{const fullDate=r.created_at?new Date(r.created_at).toLocaleDateString("ja-JP",{year:"numeric",month:"numeric",day:"numeric"})+" "+new Date(r.created_at).toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"}):"";const favContent=r.input_text&&r.output_text?`【書き起こし】\n${r.input_text}\n\n【要約】\n${r.output_text}`:r.output_text?`【要約】\n${r.output_text}`:r.input_text?`【書き起こし】\n${r.input_text}`:"";setFavModal({title:fullDate+(pid?" | ID:"+pid:""),content:favContent,recordId:r.id})}} title="お気に入りに保存" style={{padding:mob?"2px 4px":"2px 6px",borderRadius:5,border:`1px solid #f59e0b`,background:"#fffbeb",fontSize:9,fontWeight:600,color:"#92400e",fontFamily:"inherit",cursor:"pointer"}}>⭐</button>
{r.input_text&&<button onClick={()=>runQualityCheck(r)} title="AI対応品質チェック" style={{padding:mob?"2px 4px":"2px 6px",borderRadius:5,border:"1px solid #93c5fd",background:"#eff6ff",fontSize:9,fontWeight:600,color:"#2563eb",fontFamily:"inherit",cursor:"pointer"}}>🔍品質</button>}
</div>
</div>)})}
</div>
...
<div style={{fontSize:13,color:"#2a3a20",marginBottom:3,lineHeight:1.6,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontFamily:"'Zen Maru Gothic',sans-serif"}} onClick={()=>setFavDetailModal(f)}>{(f.content||"").substring(0,30)}{(f.content||"").length>30?"...":""}</div>
<div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
<button onClick={()=>{navigator.clipboard.writeText(f.content||"");sSt("📋 コピーしました")}} title="内容をコピー" style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📋</button>
<button onClick={()=>openEditModal(f)} title="タイトル・内容を編集" style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>✏️</button>
<button onClick={()=>openGenModal(f)} title="この内容から資料を生成" style={{padding:"2px 8px",borderRadius:5,border:"1px solid #93c5fd",background:"#eff6ff",fontSize:9,fontWeight:600,color:"#2563eb",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📄資料生成</button>
<button onClick={()=>setFavMoveModal(f)} title="別グループへ移動" style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📁移動</button>
<button onClick={()=>deleteFavorite(f.id)} title="お気に入りから削除" style={{padding:"2px 8px",borderRadius:5,border:"1px solid #fca5a5",background:"#fef2f2",fontSize:9,fontWeight:600,color:"#ef4444",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>🗑️</button>
</div></div>})}
</div>}
{/* お気に入り全文モーダル */}
...
{!(mob&&mobileHideItems.pip)&&<button onClick={pipActive?closePip:openPip} title="フローティング小窓を開く（PiPモード）" style={{position:"absolute",top:16,right:16,width:60,height:60,borderRadius:"50%",border:"none",background:pipActive?"#22c55e":"linear-gradient(135deg,#3f6212,#65a30d)",color:"#fff",fontSize:15,fontWeight:700,fontFamily:"inherit",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,boxShadow:pipActive?"0 0 0 3px rgba(16,185,129,.25)":"0 2px 10px rgba(20,184,166,.2)",transition:"all 0.2s"}}>
...
<button onClick={()=>{loadHist();setPage("hist")}} title="診療履歴を表示" style={{padding:mob?"4px 8px":"6px 12px",borderRadius:8,border:`1.5px solid ${C.g200}`,background:C.w,fontSize:mob?11:12,fontWeight:600,color:C.pD,fontFamily:"inherit",cursor:"pointer",boxShadow:"0 1px 3px rgba(0,0,0,.06)"}}>📂 履歴</button>
<button onClick={()=>setDictModal(true)} title="誤字脱字辞書を管理" style={{padding:mob?"4px 8px":"6px 12px",borderRadius:8,border:`1.5px solid ${C.g200}`,background:C.w,fontSize:mob?11:12,fontWeight:600,color:C.pD,fontFamily:"inherit",cursor:"pointer",boxShadow:"0 1px 3px rgba(0,0,0,.06)",whiteSpace:"nowrap"}}>📖辞書</button>
{!(mob&&mobileHideItems.fontsize)&&<div style={{display:"flex",gap:3,marginLeft:"auto"}}>
{[["small","小"],["medium","中"],["large","大"]].map(([v,l])=><button key={v} onClick={()=>setFontSize(v)} title={`文字サイズ: ${l}`} style={{padding:"2px 8px",borderRadius:7,border:"none",background:fontSize===v?"#22c55e":"#d1d5db",color:fontSize===v?"#fff":"#57534e",fontSize:11,fontWeight:700,fontFamily:"inherit",cursor:"pointer",boxShadow:"0 1px 3px rgba(0,0,0,.1)",transition:"all 0.15s"}}>{l}</button>)}
</div>}
</div>
...
{rs==="inactive"?(<button onClick={go} title="録音開始 / 停止" style={{...rb,width:mob?80:90,height:mob?80:90,background:"linear-gradient(135deg, rgba(140,210,80,0.8), rgba(180,230,100,0.75), rgba(200,240,120,0.7))",color:"#1a3a10",boxShadow:"0 4px 15px rgba(61,90,30,.3), 0 2px 4px rgba(0,0,0,.1)"}}><span style={{fontSize:mob?26:30}}>🎙</span><span style={{fontSize:mob?11:12}}>録音開始</span></button>):(<>
{rs==="recording"?(<button onClick={pause} title="録音開始 / 停止" style={{...rb,width:60,height:60,background:C.warn,color:"#78350f"}}><span style={{fontSize:22}}>⏸</span></button>):(<button onClick={resume} title="録音開始 / 停止" style={{...rb,width:60,height:60,background:C.rG,color:C.w}}><span style={{fontSize:22}}>▶</span></button>)}
<button onClick={stopSum} title="AIで要約する" style={{...rb,width:50,height:50,background:"linear-gradient(135deg, rgba(140,210,80,0.8), rgba(160,220,100,0.75))",color:"#1a3a10",boxShadow:"0 4px 14px rgba(101,163,13,.25)"}}><span style={{fontSize:14,fontWeight:700}}>✓</span><span style={{fontSize:9,fontWeight:700,color:"#1a3a10"}}>要約</span></button>
<button onClick={stop} title="録音開始 / 停止" style={{...rb,width:60,height:60,background:C.err,color:C.w}}><span style={{fontSize:22}}>⏹</span></button></>)}
</div>
{rs==="recording"&&<div style={{fontSize:12,color:C.g400}}>🎙 5秒ごとに自動書き起こし</div>}
<button onClick={()=>setSessionAudioSave(v=>{const next=v===null?!audioSave:!v;return next})} title="録音音声をSupabaseに保存する" style={{padding:"4px 12px",borderRadius:8,border:`1px solid ${(sessionAudioSave!==null?sessionAudioSave:audioSave)?C.p:C.g200}`,background:(sessionAudioSave!==null?sessionAudioSave:audioSave)?"#f0fdf4":C.g50,fontSize:11,fontWeight:600,color:(sessionAudioSave!==null?sessionAudioSave:audioSave)?"#2a4a18":C.g400,fontFamily:"inherit",cursor:"pointer"}}>🎙️音声保存 {(sessionAudioSave!==null?sessionAudioSave:audioSave)?"ON":"OFF"}</button>
</div>
<div style={{display:"flex",gap:mob?4:8,marginBottom:14,flexWrap:mob?"wrap":"nowrap"}}>
<button onClick={()=>sum()} disabled={ld||!inp.trim()} title="AIで要約する" style={{flex:1,padding:"4px 16px",borderRadius:10,border:"none",background:ld?"rgba(160,220,100,0.2)":"linear-gradient(135deg, rgba(140,210,80,0.8), rgba(180,230,100,0.75), rgba(200,240,120,0.7))",color:"#1a3a10",fontSize:11,fontWeight:700,fontFamily:"inherit",cursor:"pointer",opacity:!inp.trim()?0.4:1,boxShadow:!ld&&inp.trim()?"0 4px 15px rgba(61,90,30,.3), 0 2px 4px rgba(0,0,0,.1)":"none",transition:"all 0.2s",minWidth:60,whiteSpace:"nowrap",height:mob?44:undefined}}>{ld?"⏳ 処理中...":"⚡ 要約"}</button>
<button onClick={()=>{saveUndo();sInp("");sOut("");sSt("クリアしました")}} title="書き起こし・要約をクリア" style={{padding:"10px 16px",borderRadius:14,border:`1px solid ${C.g200}`,background:C.w,fontSize:14,fontWeight:600,color:C.g500,fontFamily:"inherit",cursor:"pointer",minWidth:44,whiteSpace:"nowrap",height:mob?44:undefined}}>🗑</button>
<button onClick={undo} title="要約を元に戻す" style={{padding:"10px 14px",borderRadius:14,border:`1px solid ${C.g200}`,background:C.w,fontSize:14,fontWeight:600,color:C.g500,fontFamily:"inherit",cursor:"pointer",opacity:undoRef.current?1:.35,minWidth:44,whiteSpace:"nowrap",height:mob?44:undefined}}>↩</button>
<button onClick={clr} title="次の患者へ" style={{padding:"10px 20px",borderRadius:14,border:`2px solid ${C.p}`,background:C.w,fontSize:14,fontWeight:700,color:C.pD,fontFamily:"inherit",cursor:"pointer",minWidth:80,whiteSpace:"nowrap",boxShadow:"0 3px 10px rgba(0,0,0,.15), 0 1px 3px rgba(0,0,0,.1)",height:mob?44:undefined}}>次へ ▶</button></div>
...
<button onClick={()=>{const favContent=inp&&out?`【書き起こし】\n${inp}\n\n【要約】\n${out}`:out?`【要約】\n${out}`:inp?`【書き起こし】\n${inp}`:"";setFavSaveModal({title:new Date().toLocaleDateString("ja-JP")+(pId?" | "+pId:""),content:favContent,recordId:""})}} title="お気に入りに保存" style={{padding:"2px 6px",borderRadius:8,border:"1px solid #f59e0b44",background:"#fffbeb",fontSize:11,fontWeight:600,color:"#92400e",fontFamily:"inherit",cursor:"pointer"}}>⭐</button></div>}
</div>
