...
<button onClick={()=>setHistPopup({title:"📝 書き起こし",content:r.input_text||"（書き起こしなし）",date,pid})} title="書き起こし内容を表示" onMouseEnter={e=>showTip(e,"書き起こし内容を表示")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📝書起</button>
<button onClick={()=>setHistPopup({title:"📋 要約",content:r.output_text||"（要約なし）",date,pid})} title="要約内容を表示" onMouseEnter={e=>showTip(e,"要約内容を表示")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.p}`,background:C.pLL,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"inherit",cursor:"pointer"}}>📋要約</button>
<button onClick={()=>{const fullDate=r.created_at?new Date(r.created_at).toLocaleDateString("ja-JP",{year:"numeric",month:"numeric",day:"numeric"})+" "+new Date(r.created_at).toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"}):"";setFavModal({title:fullDate+(pid?" | ID:"+pid:""),input_text:r.input_text||"",output_text:r.output_text||"",recordId:r.id})}} title="お気に入りに保存" onMouseEnter={e=>showTip(e,"お気に入りに保存")} onMouseLeave={hideTip} style={{padding:mob?"2px 4px":"2px 6px",borderRadius:5,border:`1px solid #f59e0b`,background:"#fffbeb",fontSize:9,fontWeight:600,color:"#92400e",fontFamily:"inherit",cursor:"pointer"}}>⭐</button>
{r.input_text&&<button onClick={()=>runQualityCheck(r)} title="AI対応品質チェック" onMouseEnter={e=>showTip(e,"AI対応品質チェック")} onMouseLeave={hideTip} style={{padding:mob?"2px 4px":"2px 6px",borderRadius:5,border:"1px solid #93c5fd",background:"#eff6ff",fontSize:9,fontWeight:600,color:"#2563eb",fontFamily:"inherit",cursor:"pointer"}}>🔍品質</button>}
</div>
</div>)})}
</div>
...
<button onClick={()=>{navigator.clipboard.writeText(f.content||"");sSt("📋 コピーしました")}} title="内容をコピー" onMouseEnter={e=>showTip(e,"内容をコピー")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📋</button>
<button onClick={()=>openEditModal(f)} title="タイトル・内容を編集" onMouseEnter={e=>showTip(e,"タイトル・内容を編集")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>✏️</button>
<button onClick={()=>openGenModal(f)} title="この内容から資料を生成" onMouseEnter={e=>showTip(e,"この内容から資料を生成")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:"1px solid #93c5fd",background:"#eff6ff",fontSize:9,fontWeight:600,color:"#2563eb",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📄資料生成</button>
<button onClick={()=>setFavMoveModal(f)} title="別グループへ移動" onMouseEnter={e=>showTip(e,"別グループへ移動")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:`1px solid ${C.g200}`,background:C.g50,fontSize:9,fontWeight:600,color:"#2a4a18",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>📁移動</button>
<button onClick={()=>deleteFavorite(f.id)} title="お気に入りから削除" onMouseEnter={e=>showTip(e,"お気に入りから削除")} onMouseLeave={hideTip} style={{padding:"2px 8px",borderRadius:5,border:"1px solid #fca5a5",background:"#fef2f2",fontSize:9,fontWeight:600,color:"#ef4444",fontFamily:"'Zen Maru Gothic',sans-serif",cursor:"pointer"}}>🗑️</button>
</div></div>})}
</div>}
{/* お気に入り全文モーダル */}
