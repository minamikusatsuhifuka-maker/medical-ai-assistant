...
<button onClick={async(e)=>{e.stopPropagation();setDailyNoiseLd(dk);sSt(`🚫 ${dk} ノイズスキャン中...`);try{const fullText=recs.map(r=>r.input_text||"").filter(Boolean).join("\n---\n").substring(0,5000);if(!fullText.trim()){sSt("書き起こしテキストがありません");return}const res=await fetch("/api/scan-noise",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:fullText,registered:noisePatterns})});if(!res.ok){sSt("ノイズスキャンエラー: "+res.status);return}const d=await res.json();if(d.error){sSt("ノイズスキャンエラー: "+d.error);return}if(!d.candidates||d.candidates.length===0){sSt("✓ 新しいノイズ候補は見つかりませんでした");return}const registeredSet=new Set(noisePatterns);const newCandidates=d.candidates.filter(c=>!registeredSet.has(c.text));if(newCandidates.length===0){sSt("✓ 新しいノイズ候補はありません（全て登録済み）");return}setNoiseCandidates(newCandidates);setNoiseModal(true);sSt("ノイズ候補が見つかりました")}catch(err){sSt("ノイズスキャンエラー: "+err.message)}finally{setDailyNoiseLd(null)}}} title="この日の診療記録をノイズスキャン" onMouseEnter={e=>showTip(e,"この日の書き起こしをノイズスキャン")} onMouseLeave={hideTip} disabled={dailyNoiseLd===dk} style={{padding:"4px 10px",borderRadius:8,border:"1px solid #fca5a5",background:dailyNoiseLd===dk?"#e5e7eb":"#fff1f2",fontSize:11,fontWeight:600,color:dailyNoiseLd===dk?C.g400:"#dc2626",fontFamily:"inherit",cursor:dailyNoiseLd===dk?"wait":"pointer",whiteSpace:"nowrap"}}>{dailyNoiseLd===dk?"🚫 スキャン中...":"🚫 ノイズスキャン"}</button>
