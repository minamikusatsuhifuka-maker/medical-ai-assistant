...
const mR=useRef(null),msR=useRef(null),acR=useRef(null),anR=useRef(null),laR=useRef(null),tR=useRef(null),cR=useRef(null),iR=useRef(""),oR=useRef("");
...
pw.document.getElementById("pip-sum").onclick=()=>{stopSum();setTimeout(pipBtnUpdate,500);const waitDone=setInterval(()=>{const d=pipRef.current;if(d&&!document.querySelector('[style*="spin"]')){clearInterval(waitDone);setTimeout(()=>{const outputText=oR.current||"";if(outputText.trim()){navigator.clipboard.writeText(outputText).then(()=>{const d2=pipRef.current;if(d2){try{const old=d2.getElementById("pip-alert");if(old)old.remove()}catch{}const div=d2.createElement("div");div.id="pip-alert";div.style.cssText="position:fixed;bottom:8px;left:50%;transform:translateX(-50%);width:90%;background:#22c55e;color:#fff;padding:8px 12px;border-radius:10px;font-size:12px;font-weight:700;z-index:9999;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3)";div.textContent="✅ 要約完了・📋コピー済み";d2.body.appendChild(div)}}).catch(()=>{})}},800)}},1500)};
