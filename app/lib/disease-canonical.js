...
5. 湿疹・皮膚炎の見出しは「〈部位〉湿疹」の形で書き、部位は次の5つから必ず1つ選ぶ。
   ...
   ※病型名のつくもの(脂漏性皮膚炎、接触皮膚炎、アトピー性皮膚炎、皮脂欠乏性湿疹、
...
8. かぶれは「接触皮膚炎」と書く。「接触性皮膚炎」とは書かない。

9. 粉瘤は「粉瘤」と書く。「表皮嚢腫」とは書かない。`;
...
// 病型名（部位正規化の対象外・独立。前置の部位語つき（殿部接触皮膚炎 等）はこの正規形に「切り出す」構造リスト）。
// ※ここは「切り出し対象の網」であり、病名の言い換えそのものは disease_aliases テーブルが単一の真実源
//   （alias-unify-editable: 1対1の言い換えは院長がUIから編集できるようDBへ移行済み。旧UNIFY_MAPは廃止）。
//   そのため表記ゆれ形（接触性皮膚炎・脂漏性湿疹・眼瞼湿疹・汗疱状湿疹・異汗性湿疹）もこの網に含め、
//   切り出し後に disease_aliases で正式名（接触皮膚炎・脂漏性皮膚炎・眼瞼皮膚炎・汗疱）へ寄せる。
// 最長一致で切り出す（find順に依存しないよう長さ降順で保持）。
export const PHENOTYPE_NAMES = [
  "脂漏性皮膚炎", "脂漏性湿疹", "接触皮膚炎", "接触性皮膚炎", "アトピー性皮膚炎", "皮脂欠乏性湿疹",
  "眼瞼皮膚炎", "眼瞼湿疹", "うっ滞性皮膚炎", "貨幣状湿疹", "日光皮膚炎", "花粉皮膚炎",
  "炎症性皮膚炎", "慢性湿疹", "汗疱状湿疹", "異汗性湿疹", "汗疱",
...
export function toCanonicalDisease(heading, aliasMap) {
  ...
  if (!key) return null;
  // 病型名で終わる見出しは病型名に切り出す（殿部接触皮膚炎→接触皮膚炎）。部位正規化より先に行う。
  // 切り出し後の言い換え（接触性皮膚炎→接触皮膚炎 等）は末尾の aliasMap（disease_aliases）が担う
  const pheno = PHENOTYPE_NAMES.find((p) => key.endsWith(p));
  if (pheno) {
    key = pheno;
