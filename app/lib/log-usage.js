...
import { calcCost, calcWhisperCost, USD_JPY } from "./pricing";
...
export async function logUsage({
  ...
  // duration_seconds が指定されていれば Whisper扱い（per-minute課金）
  let cost_usd, cost_jpy, family, model_resolved;
  if (duration_seconds !== null && duration_seconds !== undefined && Number(duration_seconds) > 0) {
    ({ cost_usd, cost_jpy, family, model_resolved } = calcWhisperCost(duration_seconds));
  } else {
    ({ cost_usd, cost_jpy, family, model_resolved } = calcCost(model, input_tokens, output_tokens));
  }
  const finalMeta = { ...(request_meta || {}), ...(duration_seconds != null ? { duration_seconds: Number(duration_seconds) } : {}) };
  ...
  try {
    const { error } = await supabase.from("api_usage_logs").insert({
      ...
      request_meta: finalMeta, success, error_message,
    });
