...
async function part1() {
  ...
  assert(c("かゆみ（皮膚瘙痒症）") === "皮膚瘙痒症" && c("かゆみ（皮膚掻痒症）") === "皮膚瘙痒症", "かゆみ（皮膚瘙痒症/皮膚掻痒症）→ どちらも皮膚瘙痒症（医学正式表記へ統一・院長決定）");
  ...
  // ===== alias-final-decisions 第1部（院長決定） =====
  const kk1 = c("多汗症（原発性腋窩多汗症）", ALIAS_PROPOSAL), kk2 = c("手汗（掌蹠多汗症）", ALIAS_PROPOSAL), kk3 = c("多汗症（原発性局所多汗症）", ALIAS_PROPOSAL);
  assert(kk1 === "腋窩多汗症" && c("脇汗", ALIAS_PROPOSAL) === "腋窩多汗症", "§2: 原発性腋窩多汗症/脇汗 → 腋窩多汗症");
  assert(kk2 === "手掌多汗症" && c("手汗", ALIAS_PROPOSAL) === "手掌多汗症", "§2: 掌蹠多汗症/手汗 → 手掌多汗症");
  assert(kk3 === "多汗症" && c("全身多汗症", ALIAS_PROPOSAL) === "多汗症", "§2: 原発性局所多汗症/全身多汗症 → 多汗症");
  assert(kk1 !== kk2 && kk2 !== kk3 && kk1 !== kk3, "§2: 多汗症3種（腋窩/手掌/多汗症）は互いに別キー");
  for (const h of ["炎症後色素沈着", "外傷後色素沈着", "爪の色素沈着"]) {
    assert(c(h, ALIAS_PROPOSAL) === "色素沈着" && c(h, ALIAS_PROPOSAL) !== c("シミ（肝斑）", ALIAS_PROPOSAL), `§3: ${h} → 色素沈着（肝斑に統合されない）`);
  }
  assert(c("日光黒子", ALIAS_PROPOSAL) === "老人性色素斑", "§3: 日光黒子 → 老人性色素斑");
  assert(c("シミ（肝斑）", ALIAS_PROPOSAL) === "肝斑" && c("イボ（脂漏性角化症）", ALIAS_PROPOSAL) === "脂漏性角化症", "§3: 肝斑は独立・脂漏性角化症はシミ系に寄せない");
  assert(c("皮脂欠乏症", ALIAS_PROPOSAL) === "皮脂欠乏性湿疹", "§4: 皮脂欠乏症 → 皮脂欠乏性湿疹");
  assert(c("ニキビ跡（尋常性ざ瘡瘢痕）", ALIAS_PROPOSAL) === "ざ瘡瘢痕" && c("ニキビ跡（ざ瘡後瘢痕）", ALIAS_PROPOSAL) === "ざ瘡瘢痕", "§5: 尋常性ざ瘡瘢痕/ざ瘡後瘢痕 → ざ瘡瘢痕");
  assert(c("ニキビ跡（尋常性ざ瘡瘢痕）", ALIAS_PROPOSAL) !== c("ニキビ（尋常性ざ瘡）", ALIAS_PROPOSAL), "§5: ざ瘡瘢痕は尋常性ざ瘡に統合されない（別疾患）");
  assert(lib.DISEASE_HEADING_PROMPT_RULES.includes("ざ瘡瘢痕") && lib.DISEASE_HEADING_PROMPT_RULES.includes("ニキビ跡"), "§5-2: プロンプト第7条（ざ瘡瘢痕の表記統一）が追加されている");
  assert(lib.DISEASE_HEADING_PROMPT_RULES.includes("「接触皮膚炎」と書く") && lib.DISEASE_HEADING_PROMPT_RULES.includes("「粉瘤」と書く"), "プロンプト第8・9条（接触皮膚炎/粉瘤の表記）が追加されている");
  assert(!lib.isNonDiseaseKey("アレルギー性鼻炎") && lib.isNonDiseaseKey("アレルギー性結膜炎"), "§1: アレルギー性鼻炎は疾患に戻し、アレルギー性結膜炎は非疾患のまま");
...
async function part2() {
  ...
  console.log(`  多汗症3種: ${["腋窩多汗症", "手掌多汗症", "多汗症"].map((k) => `${k}=${canonAlias.get(k) || 0}`).join(" / ")}`);
  console.log(`  シミ系3種: ${["肝斑", "色素沈着", "老人性色素斑"].map((k) => `${k}=${canonAlias.get(k) || 0}`).join(" / ")}`);
  console.log(`  ざ瘡瘢痕: ${canonAlias.get("ざ瘡瘢痕") || 0}件（統合前の生見出し: ${["ニキビ跡（尋常性ざ瘡瘢痕）", "ニキビ跡（ざ瘡後瘢痕）"].map((h) => `${h}=${[...raw].includes(h) ? "実在" : "なし"}`).join(" / ")}）`);
